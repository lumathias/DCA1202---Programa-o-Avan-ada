...
class Node {
public:
    Coord pos; //posicao atual
    Coord ant; //antecessor
    double h; //heuristica
    double s; //valor do passo
    double t() const; //valor total previsto

    void heuristica(const Coord& Dest);
    Node(); //construtor
    Node (Coord Pos, Coord Ant, double S, double H); //construtor específico
    bool operator==(const Coord& C) const;
};

/// Classe functor (para busca com parametro)
/// Para busca de um noh maior que um valor (parametro)
class greater_than
{
private:
  // Valor acima do qual deve ser procurado o noh
  double value;
public:
  greater_than(double t);
  bool operator()(const Node& node);
};

/// Definicao (implementacao) da classe functor
inline greater_than::greater_than(double t): value(t) {}
